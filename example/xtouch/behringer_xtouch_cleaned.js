...
function makePortPair(driver2, isExtender) {
    ...
    var input = driver2.mPorts.makeMidiInput(
        'Input '.concat(portPairIndex.toString(), ' - ').concat(name)
    )

    var output = driver2.mPorts.makeMidiOutput(
        'Output '.concat(portPairIndex.toString(), ' - ').concat(name)
    )

    output.sendSysex = function (context, messageBody) {
        output.sendMidi(
            context,
            [0xf0, 0x0, 0x0, 0x66, 0x14 + +isExtender].concat(messageBody, [0xf7])
        )
    }
...
function setupDeviceConnection(driver, devices) {
    var activationCallbacks2 = makeCallbackCollection(driver, 'mOnActivate')
    var segmentDisplayManager2 = new SegmentDisplayManager(devices)
...
function bindCursorValueControlButton(page, button, encoder, jogWheel) {
    ...
    var encoderDisplayMode = page.mCustom.makeHostValueVariable(
    ...
    page.makeActionBinding(button.mSurfaceValue, activeSubpage.mAction.mActivate).setSubPage(
        inactiveSubpage
    )
    page.makeActionBinding(button.mSurfaceValue, inactiveSubpage.mAction.mActivate).setSubPage(
        activeSubpage
    )
...
function bindControlButtons(page, controlSectionElements, channelElements, mixerBankZone) {
    ...
    page.makeCommandBinding(buttons.edit.mSurfaceValue, 'Edit', 'Edit Channel Settings').setSubPage(
        regularSubPage
    )
    page.makeCommandBinding(
    ...
    page.makeCommandBinding(buttons.modify[0].mSurfaceValue, 'Edit', 'History').setSubPage(
        shiftSubPage
    )
    ...
    page.makeCommandBinding(buttons.modify[2].mSurfaceValue, 'File', 'Save New Version').setSubPage(
        shiftSubPage
    )
    ...
    page.makeCommandBinding(buttons.utility[2].mSurfaceValue, 'Edit', 'Unmute All').setSubPage(
        shiftSubPage
    )
...
function bindDirectionButtons(page, controlSectionElements) {
    ...
    page.makeCommandBinding(directions.up.mSurfaceValue, 'Zoom', 'Zoom Out Vertically').setSubPage(
        zoomSubPage
    )
    page.makeCommandBinding(directions.down.mSurfaceValue, 'Navigate', 'Down').setSubPage(
        navigateSubPage
    )
    page.makeCommandBinding(directions.down.mSurfaceValue, 'Zoom', 'Zoom In Vertically').setSubPage(
        zoomSubPage
    )
    page.makeCommandBinding(directions.left.mSurfaceValue, 'Navigate', 'Left').setSubPage(
        navigateSubPage
    )
    page.makeCommandBinding(directions.left.mSurfaceValue, 'Zoom', 'Zoom Out').setSubPage(
        zoomSubPage
    )
    page.makeCommandBinding(directions.right.mSurfaceValue, 'Navigate', 'Right').setSubPage(
        navigateSubPage
    )
    page.makeCommandBinding(directions.right.mSurfaceValue, 'Zoom', 'Zoom In').setSubPage(
        zoomSubPage
    )
