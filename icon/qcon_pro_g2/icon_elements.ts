...
const updateDisplay = (
    ...
    if (displayType === 'Pan') {
        ...
        if (
            newAltRow1 !== prevAltRow1 ||
            newAltRow2 !== prevAltRow2 ||
...
const bindButton2CC = (
    ...
    return btn.mSurfaceValue.mMidiBinding
        .setInputPort(midiInput)
        .bindToControlChange(channelNumber, controlChangeNumber)
}
...
const makeLedButton = (
    ...
    btn.mSurfaceValue.mOnProcessValueChange = function (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) {
...
const makeTouchFader = (
    ...
    // Fader Touch + Fader
    touchFader.btnFaderTouch = makeButton(
        surface,
        midiInput,
        midiOutput,
        x,
    ...
    touchFader.fdrFader.mSurfaceValue.mMidiBinding
        .setInputPort(midiInput)
...
const repeatCommand = (
    activeDevice: MR_ActiveDevice,
    command: MR_SurfaceCustomValueVariable,
    repeats: number
) => {
...
const makeChannelControl = (
    ...
    channelControl.pushEncoder.mEncoderValue.mOnProcessValueChange = (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) => {
        ...
        const isCenterLedOn =
            value === (displayMode === VPOT_DISPLAY_TYPE.Spread /* Spread */ ? 0 : 0.5)
        const position =
            1 + Math.round(value * (displayMode === VPOT_DISPLAY_TYPE.Spread /* Spread */ ? 5 : 10))
    ...
    channelControl.pushEncoder.mEncoderValue.mOnTitleChange = (
        activeDevice: MR_ActiveDevice,
        objectTitle: string,
        valueTitle: string
    ) => {
    ...
    channelControl.vuMeter.mOnProcessValueChange = (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) => {
        ...
        if (now - lastMeterUpdateTime > 125) {
            ...
            midiOutput.sendMidi(activeDevice, [
                0xd0,
                (channelIndex << 4) + Math.ceil(value * 14 - 0.25),
            ])
        }
    }
    ...
    const touchFader = makeTouchFader(
        surface,
        midiInput,
        midiOutput,
        fader_x,
    ...
    channelControl.btnFaderTouch.mSurfaceValue.mOnProcessValueChange = (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) => {
    ...
    channelControl.fdrFader.mSurfaceValue.mOnTitleChange = (
        activeDevice: MR_ActiveDevice,
        objectTitle: string,
        valueTitle: string
    ) => {
    ...
    channelControl.fdrFader.mSurfaceValue.mOnDisplayValueChange = (
        activeDevice: MR_ActiveDevice,
        value: string,
        units: string
    ) => {
    ...
    channelControl.pushEncoder.mEncoderValue.mOnTitleChange = (
        ...
        switch (activePage) {
            ...
            default:
                activeDevice.setState(
                    activePage + ' - Pan - Title',
                    setTextOfColumn(channelIndex, makeLabel(objectTitle, 6), panTitles)
                )
                activeDevice.setState(
                    activePage + ' - Pan - ValueTitles',
    ...
    channelControl.pushEncoder.mEncoderValue.mOnDisplayValueChange = (
        activeDevice: MR_ActiveDevice,
        value: string,
        units: string
    ) => {
        ...
        activeDevice.setState(
            activePage + ' - Pan - Values',
            setTextOfColumn(channelIndex, makeLabel(value, 6), panValues)
        )
        updateDisplay(
            activeDevice.getState('Display - idRow1'),
...
const makeMasterControl = (
    ...
    masterControl.btnFlip = makeButton(
        surface,
        midiInput,
        midiOutput,
        start_x,
    ...
    masterControl.btnChannelLeft = makeLedButton(
        surface,
        midiInput,
        midiOutput,
        start_x,
    ...
    masterControl.btnChannelRight = makeLedButton(
        surface,
        midiInput,
        midiOutput,
        start_x,
    ...
    masterControl.btnBankLeft = makeLedButton(
        surface,
        midiInput,
        midiOutput,
        start_x,
    ...
    masterControl.btnBankRight = makeLedButton(
        surface,
        midiInput,
        midiOutput,
        start_x,
    ...
    masterControl.fdrFader.mSurfaceValue.mOnTitleChange = (
        activeDevice: MR_ActiveDevice,
        objectTitle: string,
        valueTitle: string
    ) => {
    ...
    masterControl.fdrFader.mSurfaceValue.mOnDisplayValueChange = (
        activeDevice: MR_ActiveDevice,
        value: string,
        units: string
    ) => {
    ...
    masterControl.btnFaderTouch.mSurfaceValue.mOnProcessValueChange = (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) => {
        ...
        if (diff == -1) {
            ...
            activeDevice.setState(
                'MasterFader - stashRow1',
                activeDevice.getState('Display - idRow1')
            )
            activeDevice.setState(
                'MasterFader - stashRow2',
                activeDevice.getState('Display - idRow2')
            )
            activeDevice.setState(
                'MasterFader - stashAltRow1',
                activeDevice.getState('Display - idAltRow1')
            )
            activeDevice.setState(
                'MasterFader - stashAltRow2',
                activeDevice.getState('Display - idAltRow2')
            )
            updateDisplay(
                'MasterFader - Title',
    ...
    masterControl.btnFlip.mSurfaceValue.mOnProcessValueChange = (
        activeDevice: MR_ActiveDevice,
        value: number,
        diff: number
    ) => {
...
const makeTransport = (
    ...
    bindCommandKnob(
        transport.knobJogWheel.mSurfaceValue,
        transport.jogRightVariable,
        transport.jogLeftVariable
    )
